...
public:
    ...
    bool GetNextFrame(uint8_t *pDst, int nDstPitch = 0, bool bDevice = false, CUstream stream=0) {
        if (!bReady) {
            return false;
        }
...
private:
    bool DecodeNextFrame(CUstream stream=0) {
        AVPacket *pkt = NULL;
        do {
            pDemuxer->Demux(&pkt);
            uint8_t **ppFrame = NULL;
            NvFrameInfo *pInfo = NULL;
            int nFrameDecoded = pDec->DecodeLockFrame(pkt->data, pkt->size, &ppFrame, &pInfo, 0, 0, stream);
