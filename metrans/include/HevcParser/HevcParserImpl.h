...
namespace HEVC
{
  class HevcParserImpl: public Parser
  {
    ...
    protected:
      std::vector<Data_NALUnit> parse(const uint8_t *pdata, std::size_t size, std::size_t offset);
      std::shared_ptr<NALUnit> processNALUnit(const uint8_t *pdata, std::size_t size, const Parser::Info &info);
