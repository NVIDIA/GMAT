...
public:
    ...
    AVFilterContext* GetInFilterCtx() {
        return m_filterIn;
    }
    ...
    void ParseFilter(AVFilterGraph *filterGraph, const char *szFilterDesc) {
        ...
        if (m_ffFrameCtx) {
            for (int i = 0; i < m_filterGraph->nb_filters; i++) {
                m_filterGraph->filters[i]->hw_device_ctx =
                    av_buffer_ref(((AVHWFramesContext*)m_ffFrameCtx->data)->device_ref);
                if (!m_filterGraph->filters[i]->hw_device_ctx) {
                    LOG(ERROR) << "Null device context\n";
                    return;
                }
            }
        }
