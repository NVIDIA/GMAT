...
public:
    VidEnc(AVPixelFormat eInputFormat, int nWidth, int nHeight, const char *szCodecName, 
        AVCodecID eCodecId = AV_CODEC_ID_MPEG4, AVRational timebase = {}, int nFps = 25, 
        const char *szCodecParam = NULL, bool usePrimCuCtx = false) 
    {
        ...
        std::string codecString{codec->name};
        if (usePrimCuCtx && (codecString.find("nvenc") != std::string::npos)) {
            int err = 0;
            if ((err = av_hwdevice_ctx_create(&m_hwDeviceCtx, AV_HWDEVICE_TYPE_CUDA,
                                      NULL, NULL, 1)) < 0) {
                fprintf(stderr, "Failed to create specified HW device.\n");
            }
            m_enc->hw_device_ctx = av_buffer_ref(m_hwDeviceCtx);
        }
        ...
        // DEBUG: print cuda context
        // AVCUDADeviceContext *device_hwctx = (AVCUDADeviceContext*)(((AVHWDeviceContext*)m_enc->hw_device_ctx->data)->hwctx);
        // CUcontext current = device_hwctx->cuda_ctx;
    ...
    // VidEnc(AVPixelFormat eInputFormat, int nWidth, int nHeight, const char *szCodecName, bool usePrimCuCtx,
...
private:
    ...
    int64_t m_pts = 0;
    AVBufferRef *m_hwDeviceCtx = NULL;
};
