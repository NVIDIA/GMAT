...
struct Data_NALUnit {
	const uint8_t *pData;
	std::shared_ptr<NALUnit> pNALUnit;
};
...
public:
	...
	enum WarningType {
		NONE, OUT_OF_RANGE, REFERENCE_STRUCT_NOT_PRESENT, PROFILE_CONFORMANCE
	};
	...
	virtual std::vector<Data_NALUnit> parse(const uint8_t *pdata, std::size_t size, std::size_t offset = 0) = 0;
