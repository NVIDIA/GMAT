...
std::vector<Data_NALUnit> HevcParserImpl::parse(const uint8_t *pData, std::size_t size, std::size_t offset) {
    std::vector<Data_NALUnit> ret;
    bool parseFailed = false;
    for (std::size_t pos = 0; pos + 3 < size;) {
        std::size_t startOffset;
        if (pData[pos] == 0 && pData[pos + 1] == 0 && pData[pos + 2] == 1) {
            startOffset = 3;
        } else if (size - pos >= 4 && pData[pos] == 0 && pData[pos + 1] == 0 && pData[pos + 2] == 0
                && pData[pos + 3] == 1) {
            startOffset = 4;
        } else {
            pos++;
        ...
        Data_NALUnit dn = {pData + pos};
        Parser::Info info;
        info.m_position = offset + pos;
        try {
            dn.pNALUnit = processNALUnit(pData + pos + startOffset, size - pos - startOffset, info);
            ret.push_back(dn);
        } catch (std::runtime_error &err) {
            std::cerr << pos << " " << err.what();
            parseFailed = true;
...
std::shared_ptr<NALUnit> HevcParserImpl::processNALUnit(const uint8_t *pdata, std::size_t size, const Parser::Info &info)
{
